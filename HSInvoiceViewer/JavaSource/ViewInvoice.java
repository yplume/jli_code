...
public class ViewInvoice extends HttpServlet implements com.ibm.mm.sdk.common.DKConstantICM
{
  ...
  public synchronized void doGet(HttpServletRequest httpservletrequest, HttpServletResponse httpservletresponse) throws ServletException, IOException
  {
    ...
    try
    {
      ...
      connection.connect();
      String query = null;
      String barcode = httpservletrequest.getParameter("BarCode");
      ...
      query = "/" + itemtype + "[@BarCode=\"" + barcode + "\"]";
	  ...
	  while ((ddo = cursor.fetchNext()) != null)	
	  {
		...
		//	if(count==cursor.cardinality()){
		  ...
		  System.out.println("pid in ViewInv = " + pid);
          ...
          {*/
            ...
            //if cursor > 1 then show list of files
            if(cursor.cardinality() > 1 && pid == null){
            	
            	pw.println("<tr>");
        		pw.println("<td>");
        		pw.println("<a href='http://usnymetia5:235/HSInvoiceViewer/ViewInvoice?itemtype=" + itemtype + "&BarCode="+ barcode +"&pid=" +pidx+ "'>"+docPart.getOriginalFileName().substring(index + 1)+"</a><br/>");
        		pw.println("</td>");
        		pw.println("</tr>");
	  ...
	  if(cursor.cardinality()==0) {
        httpservletresponse.sendRedirect("http://usnymetia5:235/ViewInvoice/NotFound.jsp?action=view&BarCode=" + httpservletrequest.getParameter("BarCode"));
      }
	  httpservletresponse.flushBuffer();
