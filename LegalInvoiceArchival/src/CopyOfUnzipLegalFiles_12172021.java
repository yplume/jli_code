...
public class CopyOfUnzipLegalFiles_12172021 {
	...
	public CopyOfUnzipLegalFiles_12172021(){
    ...
    private  void parsezipfolder(String zipFilePath, String unzipFileDir, String newDestDir, String newDestKofaxDir){
    	...
    	try{
	    	...
	    	for (File file1 : fList){
	    		...
	    		xlszipName = zipFilePath + "\\" + zipFileName;
	    		//check if xls or zipfile, if it's xls file then parse it
                if(xlszipName.trim().length()!=0 && (xlszipName.endsWith("xlsx") || xlszipName.endsWith("xls"))) {
  	          	  
  	              inputStream = new FileInputStream(new File(xlszipName));
              
          		  Workbook wb = null;
              	  if (xlszipName.endsWith("xlsx")) {
              	    wb = new XSSFWorkbook(inputStream);
              	  } else if (xlszipName.endsWith("xls")) {
              	 ...
              	 for (int i = 1; i < sh.getLastRowNum(); i++){
              		 text = sh..getRange().get(i + 1, 1).getText();

              	 }*/
              	Iterator<Row> rowIterator = sh.rowIterator();
                
                System.out.println("[UnzipLegalTracker]rowIterator = " + rowIterator);
                ...
                DataFormatter formatter = new DataFormatter(); 
              	...
              	for (; rowIterator.hasNext(); cellIterator.hasNext()){
	              	...
	              	System.out.println("[UnzipLegalTracker]cellIterator = " + cellIterator);
	              	...
	              	System.out.println("[UnzipLegalTracker]val = " + val);
	                //parse each first column of the sheet
	              	...
	              	if(splitText[0].equals("L")){
	              		...
	              		System.out.println("[UnzipLegalTracker]AL0 lenght = " + (metaAL.size()));
	    	...
	    	for (File file : fList){
	            if (file.isFile()){
	            	...
	            	System.out.println("[UnzipLegalTracker]AL = "  + metaAL.get(1));
	            	...
	            	System.out.println("[UnzipLegalTracker]AL5 = "  + test[5]);
	            	
	            	zipFileName = file.getName();
		    		
		    		xlszipName = zipFilePath + "\\" + zipFileName;
		    		System.out.println("[UnzipLegalTracker]zipFileName = "  + zipFileName);
		    		System.out.println("[UnzipLegalTracker]xlszipName = "  + xlszipName);
	            	
		    		//check if xls or zipfile, if it's xls file then parse it
	                if(xlszipName.trim().length()!=0 && (xlszipName.endsWith("zip"))) {
...
//    		
    	...
    	//if InvoiceSubmissionMethod not equal PaperViaICS
           ...
           */
            ...
            double amount = convertDecimals(metaData[11]);
	    	...
	    	*/
	    	...
	    	for(int i = 0; i<pdfFileNameAL.size(); i++){
				...
				sb.append("\t\t\t\t\t\t<IndexField Name=\"Vendor Name\" Value=\"" + metaData[9] + "\"/>\n");
				sb.append("\t\t\t\t\t\t<IndexField Name=\"Vendor Number\" Value=\"" + metaData[8] + "\"/>\n");
				sb.append("\t\t\t\t\t\t<IndexField Name=\"Invoice Number\" Value=\"" + metaData[5] + "\"/>\n");
				...
				//for(int i = 0; i<pdfFileNameAL.size(); i++){
				sb.append("\t\t\t\t\t\t<Page ImportFileName=\"" + "\\\\usncdvkfxcrtr2v\\ExportToKofax\\LOnlineImages\\" + pdfFileNameAL.get(i).getName()  + "\"/>\n");
    ...
    public static void main (String[] arg){
    	CopyOfUnzipLegalFiles_12172021 uz = new CopyOfUnzipLegalFiles_12172021();
