...
public class UnzipLegalFiles_Prod {
	...
	//private static String zipErrDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\Ziperror\\";
	//private static String zipFileDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\ZipInput\\";
	...
	///private static String zipErrDir = "C:\\ZipTest\\Ziperror\\";
	////private static String zipFileDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\LegalZipinput\\";
	////private static String zipErrDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\LegalzipErr\\";
	private static String zipFileDir = "\\\\usncpdktmrtr3v\\ExportToKofax\\LZipinput\\";
	private static String zipErrDir = "\\\\usncpdktmrtr3v\\ExportToKofax\\LegalzipErr\\";
	public UnzipLegalFiles_Prod(){
	...
	public void UnzipFiles1() {
		...
		//String unzipFileDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\ZipOutput\\";
        
        //File zipErrFile = new File("\\\\usncdvkfxcrtr2v\\ExportToKofax\\Ziperror\\");
		...
		////String unzipFileDir = "\\\\usncdvkfxcrtr2v\\LegalZipOutput\\";
        
		////File zipErrFile = new File("\\\\usncdvkfxcrtr2v\\LegalzipErr\\");
        String unzipFileDir = "\\\\usncpdktmrtr3v\\ExportToKofax\\LZipOutput\\";
        ...
        //String newDestDir = "\\\\usncdvkfxcrtr2v\\ExportToKofax\\OnlineImages\\";
        ...
        //FileUtils.copyDirectory(new File(zipFileDir), new File("\\\\usncpdktmrtr3v\\ExportToKofax\\ZipBackup\\"));
    ...
    private  void parsezipfolder(String zipFilePath, String unzipFileDir, String newDestDir, String newDestKofaxDir){
    	...
    	try{
	    	...
	    	ArrayList<String[]> metaAL = new ArrayList<String[]>();
	    	...
	    	for (File file1 : fList){
                ...
                System.out.println("[UnzipLegalTracker]CSV csvZipName = " + csvZipName);
    		        
	    		//process csv file
	    		ArrayList records = new ArrayList();
	    		try {
	    			//BufferedReader br = new BufferedReader(new FileReader("C:\\ZipTest\\FieldHighlight_Tracker Invoice Spreadsheet.1040.2021-11-11 to 2021-11-11.csv"));
	    			BufferedReader br = new BufferedReader(new FileReader(csvZipName));
	    		    String line;
	    		    while ((line = br.readLine()) != null) {
	    		        
	    		        String[] splitText = line.split("\\|");
	    		        System.out.println("[UnzipLegalTracker]CSV splitText size = " + splitText.length);
	    	...
	    	for (File file : fList){
	            if (file.isFile()){
		    		...
		    		System.out.println("[UnzipLegalTracker]cvsZipName = "  + csvZipName);
    ...
    public  void findpdffiles(ArrayList AL, String unzipFileDir, String newXMLDir, String newKofaxXMLDir){
    	...
    	try{
	    	...
	    	//for(int i=0; i<=fList.length; i++){
	    	System.out.println("ArryList = "+AL.size());
    		...
    		if (AL != null && AL.size() != 0) {
    			...
    			//for(int i = 0; i < fList.length; i++){
	        		...
	        		metaDate = (String[])AL.get(0);
					//fileName1 = metaDate[4].replace("/", "");
	        		fileName1 = metaDate[27];
	        		int invIndex = 0;
	        		invIndex = fileName1.lastIndexOf(" - ");
	        		fileName1 = fileName1.substring(invIndex+3);
	        		//System.out.println("fList = "+ fList[i]);
	        		System.out.println("FileName1 = "+ fileName1);
		    		System.out.println("metaDate5 = "+ ((String[])AL.get(0))[5]);
		    		...
		    		//if(pos >= 0){
			    		...
			    		for(int k = 0; k < fList.length; k++){
				    		...
				    		if(isPDF && fileName1.equalsIgnoreCase(fileName2.substring(pos1 + 3, pos))){
				    		//if(isPDF && fileName2.endsWith(fileName1 + ".pdf")){
				    			...
				    			//if(fileName2.equalsIgnoreCase(fileName1)){
				    				...
				    				//System.out.println(((String[])AL.get(count))[4] + "***" + count);
				    				AL.remove(0);
				    				i = 0;
					    			...
					    			System.out.println("fileName1111==="+extractDirectory + "\\" + fileName2);
				    				
					    			System.out.println("fileName2222==="+newXMLDir + fileName2);
				    	    		...
				    	    		createNewXML(metaDate, newKofaxXMLDir + fileName2 + ".xml", newKofaxXMLDir, attachAL, fileName1);
    ...
    public  void createNewXML(String[] metaData, String newKofaxXml, String KofaxXMLDir, ArrayList <File> pdfFileNameAL, String invNum){
    	...
    	try{
	    	...
	    	*/
	    	...
	    	for(int i = 0; i<pdfFileNameAL.size(); i++){
				...
				sb.append("\t\t\t\t\t\t<IndexField Name=\"Vendor Name\" Value=\"" + metaData[8].replaceAll("[^\\.A-Za-z0-9_& ]", "").replace("&", "&amp;").toString().trim() + "\"/>\n");
                sb.append("\t\t\t\t\t\t<IndexField Name=\"Vendor Number\" Value=\"" + metaData[7].replaceAll("[^\\.A-Za-z0-9_& ]", "").replace("&", "&amp;").toString().trim() + "\"/>\n");
                sb.append("\t\t\t\t\t\t<IndexField Name=\"Invoice Number\" Value=\"" + invNum + "\"/>\n");
                //sb.append("\t\t\t\t\t\t<IndexField Name=\"Invoice Number\" Value=\"" + metaData[4].replaceAll("[^\\.A-Za-z0-9_&\\- ]", "").replace("&", "&amp;").toString().trim() + "\"/>\n");
				...
				//for(int i = 0; i<pdfFileNameAL.size(); i++){
				//sb.append("\t\t\t\t\t\t<Page ImportFileName=\"" + "\\\\usncdvkfxcrtr2v\\ExportToKofax\\OnlineImages\\" + pdfFileNameAL.get(i).getName()  + "\"/>\n");
				sb.append("\t\t\t\t\t\t<Page ImportFileName=\"\\\\usncpdktmrtr3v\\ExportToKofax\\LOnlineImages\\" + pdfFileNameAL.get(i).getName().replace("&", "&amp;").toString().trim() + "\"/>\n");
    ...
    public static void main (String[] arg){
    	UnzipLegalFiles_Prod uz = new UnzipLegalFiles_Prod();
