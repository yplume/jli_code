...
public class UnzipFiles_DEV {
	...
	public void UnzipFiles1() {
        ...
        FileUtils.copyDirectory(new File(zipFileDir), new File("\\\\usncdvkfxcrtr2v\\ExportToKofax\\ZipBackup\\"));
    	...
    	try{
	    	...
	    	for(int i=0; i<fList.length; i++){
	    		...
	    		if(pos >= 0){
		    		...
		    		for(int k=i+1; k<fList.length; k++){
		    			int pos1 = 0;
    ...
    //public static void createNewXML(XMLReader xmlreader, String unzipedXml, String newKofaxXml, String cXML, ArrayList <File> pdfFileNameAL){
    	...
    	if(!xmlreader.getInvoiceSubmissionMethod().equalsIgnoreCase("PaperViaICS")){
	    	...
	    	String vendorName = xmlreader.getVendorName().replaceAll("[^\\.A-Za-z0-9_& ]", "");
	    	String invNum = xmlreader.getInvoiceID().replaceAll("[^\\.A-Za-z0-9_& ]", "");
	    	String payeeName = xmlreader.getPayeeName().replaceAll("[^\\.A-Za-z0-9_& ]", "");
    ...
    private static void unzip(String zipFilePath, String destDir, String newDir) {
        ...
        try {
            ...
            while(ze != null){
                ...
                //will fix error 'The filename, directory name, or volume label syntax is incorrect.'
