 ...
 * web browser.
 */
public class SearchStatusFilterResponse extends PluginResponseFilter {
          ...
          //Disable Insert Status column and add Kofax information
			...
			for (int i = 0; i < jsonResultSetResponse.getRowCount(); i++) {
			    ...
			    //System.out.println("conn0="+conn0);
						...
						//populate value from USNCPDKFXCAP1v server		
					    if(rs.next()) {
					//while (rs.next()) {
						    ...
						    rs.close();
				      	//}	
					    }else{ //populate value from USNCDVKTMRTR2V server		
					    	conn1 = DriverManager.getConnection("jdbc:sqlserver://USNCDVKTMRTR2V;DatabaseName=ExportTestNew;user=test;password=test;integratedSecurity=false");
					    	if (conn1 != null) {
					    		stmt1 = conn1.createStatement();
					    		String sql1 = "SELECT Validator, ScanDateTime, BatchName FROM dbo.ExportAriba where RepositoryDocID like '%" + pidLike + "%'";
					       	 	rs1 = stmt1.executeQuery(sql1);
					    	}
					    	validator = rs1.getString("Validator");
				      		scanTime  = rs1.getString("ScanDateTime");
							BatchName = rs1.getString("BatchName");
							if (validator == null) validator = "";
							if (scanTime == null) scanTime = "";
							if (BatchName == null) BatchName = "";
